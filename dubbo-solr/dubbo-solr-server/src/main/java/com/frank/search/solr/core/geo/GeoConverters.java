 ...
 */
package com.frank.search.solr.core.geo;
...
public final class GeoConverters {
	...
	public enum Point2DToStringConverter implements Converter<org.springframework.data.geo.Point, String> {
		...
		public String convert(org.springframework.data.geo.Point source) {
			...
			String formattedString = StringUtils.stripEnd(String.format(java.util.Locale.ENGLISH, "%f", source.getX()),
					"0") + ","
	...
	/**
	 * Converts a {@link org.springframework.data.geo.Distance} to a
	 * solrReadable request parameter.
	 */
	@WritingConverter
		...
		public String convert(org.springframework.data.geo.Point source) {
			...
			if (source instanceof Point) {
				formattedString += (((Point) source).getZ() != null
						? ("," + StringUtils
								.stripEnd(String.format(java.util.Locale.ENGLISH, "%f", ((Point) source).getZ()), "0"))
						: "");
			}
