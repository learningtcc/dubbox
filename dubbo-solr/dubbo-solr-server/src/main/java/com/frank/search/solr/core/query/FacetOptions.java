...
public class FacetOptions {
	 ...
	 * 
	 * @param field
	 *            the {@link Field} to be appended to range faceting fields
	 * @return this
	 * @since 1.5
	 */
	...
	/**
	 * Set minimum number of hits {@code facet.mincount} for result to be
	 ...
	 * @author Francisco Spaeth
	 * @param <T>
	 *            range field implementation type
	 * @param <R>
	 *            type of range
	 * @param <G>
	 *            type of gap
	...
	public abstract static class FieldWithRangeParameters<T extends FieldWithRangeParameters<?, ?, ?>, R, G>
		 ...
		 *            range facet end
		 * @param gap
		 *            gap to be used for faceting between start and end
		 */
		...
		/**
		 * Defines if the last range should be abruptly ended even if the end
		 * doesn't satisfies: (start - end) % gap = 0.
		 * 
		 * @param rangeHardEnd
		 *            whenever <code>false</code> will expect to have the last
		 *            range with the same size as the other ranges entries for
		 *            the query, otherwise (<code>true</code>), may present the
		 *            last range smaller than the other range entries.
		 * @return this
		 * @see org.apache.solr.common.params.FacetParams#FACET_RANGE_HARD_END
		 */
		@SuppressWarnings("unchecked")
		...
		/**
		 * If the last range should be abruptly ended even if the end doesn't
		 * satisfies: (start - end) % gap = 0.
		 *
		 * @return if hard end should be used, <code>null</code> will be
		 *         returned if not set
		 * @see org.apache.solr.common.params.FacetParams#FACET_RANGE_HARD_END
		 */
		...
		/**
		 * Defines the additional (other) counts for the range facet, i.e. count
		 * of documents that are before start of the range facet, end of range
		 * facet or even between start and end.
		 *
		 * @param rangeOther
		 *            which other counts shall be added to the facet result
		 * @return this
		 * @see org.apache.solr.common.params.FacetParams.FACET_RANGE_OTHER
		 */
		@SuppressWarnings("unchecked")
		public T setOther(FacetRangeOther rangeOther) {
		...
		/**
		 * Defines how boundaries (lower and upper) shall be handled (exclusive
		 * or inclusive) on range facet requests.
		 *
		 * @param rangeInclude
		 *            include option for range
		 * @return this
		 * @see org.apache.solr.common.params.FacetParams.FACET_RANGE_INCLUDE
		 */
		@SuppressWarnings("unchecked")
		public T setInclude(FacetRangeInclude rangeInclude) {
			addFacetRangeParameter(FacetParams.FACET_RANGE_INCLUDE, rangeInclude);
			return (T) this;
		}

		/**
		 * The definition of how boundaries (lower and upper) shall be handled
		 * (exclusive or inclusive) on range facet requests.
		 *
		 * @return null if not set
		 * @see org.apache.solr.common.params.FacetParams.FACET_RANGE_INCLUDE
		 */
		 ...
		 * @see org.apache.solr.common.params.FacetParams#FACET_RANGE_GAP
		 */
		public G getGap() {
		 ...
		 * @see org.apache.solr.common.params.FacetParams#FACET_RANGE_START
		 */
		public R getStart() {
		 ...
		 * @see org.apache.solr.common.params.FacetParams#FACET_RANGE_END
		 */
		public R getEnd() {
