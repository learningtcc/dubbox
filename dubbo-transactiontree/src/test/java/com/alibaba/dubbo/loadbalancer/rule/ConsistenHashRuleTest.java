...
public class ConsistenHashRuleTest {
	
	private Random random = new Random();

	private BaseLoadBalancer lb;
	private ConsistentHashRule rule;
	private Ping ping;

	@BeforeClass
	public void beforeClass() {
		rule = new ConsistentHashRule();
...
//				System.out.println("serverListChanged " + toServerString(newList));
			}
		});
		lb.addServerStatusChangeListener(new ServerStatusChangeListener() {
			...
			public void serverStatusChanged(Collection<Server> servers) {
				...
				// toServerString(servers));
//				System.out.println("serverStatusChanged1 " + toServerString(servers));
//				 System.out.println("serverStatusChanged2 " +
//				 lb.getServerList(true));
			}
		});
		lb.addServerStatusChangeListener(rule.getServerStatusChangeListener());
	...
	public void test1() throws InterruptedException {
		Executor e = Executors.newFixedThreadPool(500);
...
//							e.printStackTrace();
//						}
						String uuid = random.nextInt(100) + "";
